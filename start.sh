...
################################################################################
#                       2.5️⃣  ЗАПУСК МОБІЛЬНОГО ДОДАТКА                      #
################################################################################

print_header "📱 КРОК 2.5: ЗАПУСК МОБІЛЬНОГО ДОДАТКА (Expo)"

if [ -d "$PROJECT_DIR/mobile" ]; then
    print_step "Запускаю Expo Server у папці /mobile..."
    cd "$PROJECT_DIR/mobile" || exit 1
    npx expo start -c --tunnel --non-interactive > /tmp/expo_mobile.log 2>&1 &
    EXPO_PID=$!
    cd "$PROJECT_DIR" || exit 1
    print_success "Expo запущено у фоні (Логи: tail -f /tmp/expo_mobile.log)"
else
    print_error "Папка /mobile не знайдена. Пропускаю запуск додатка."
fi
